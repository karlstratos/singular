...
def read_normalized_embeddings(embedding_path, vocab):
    """Read (and normalize) embeddings from the given path."""
    embedding = {}
    dim = 0
...
def predict(w1, w2, v1, embedding, method):
    """
    Finds x in vocab V such that w1:w2 as in v1:x using embeddings.
    """
    if method == "add":
        # argmax_{x in V \ {w1, w2, v1}} cos(x, w2 - w1 + v1)
        target_embedding = embedding[w2] - embedding[w1] + embedding[v1]
        target_embedding /= linalg.norm(target_embedding)
    elif method == "raw":
        # argmax_{x in V \ {w1, w2, v1}} cos(x, v1)
        target_embedding = embedding[v1]
    elif method == "mult":
    ...
    for word in embedding:
        if (word == w1) or (word == w2) or (word == v1):
            # Do not consider words that appear in the question.
            continue
...
def evaluate_analogy(analogy_path, embedding_path, method):
    ...
    # Read normalized embeddings for word types we need.
    embedding, dim = read_normalized_embeddings(embedding_path, vocab)
    print "{0} / {1} words have embeddings (dim {2})\n".format(
    ...
    for analogy_type in questions:
        for (w1, w2, v1, v2) in questions[analogy_type]:
            if (w1 in embedding) and (w2 in embedding) and (v1 in embedding) \
                   and (v2 in embedding):
                num_answered += 1
                ...
                predicted_v2 = predict(w1, w2, v1, embedding, method)
                if predicted_v2 == v2:
                    num_correct += 1
